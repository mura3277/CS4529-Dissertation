...
from dissertation import run_func_profiled, graph_and_log_profiled_solutions, SolutionType, print_total_runs_and_time
...
#Main scan running method
def run_scan():
    with log_terminal_output(log_dir) as log_file:
        #Print details about the current configuration
        print("Scene:", scene_1)
...
run_func_profiled(run_scan, iterations, [SolutionType.CHAT_GPT_CODE, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.NO_LIST_COMP, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.INIT_NP_ARRAY, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_DTYPE, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_INDEXING, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_VIEWS, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_RAW, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_DICT, SolutionType.ORIG_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_CLOOP, SolutionType.ORIG_INTER], graph=False)

run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_CLOOP, SolutionType.NO_SELF_INTER], graph=False)
run_func_profiled(run_scan, iterations, [SolutionType.CYTHON_CLOOP, SolutionType.CYTHON_INTER], graph=False)

#Generate graph of profiled solutions
graph_and_log_profiled_solutions()
print_total_runs_and_time()